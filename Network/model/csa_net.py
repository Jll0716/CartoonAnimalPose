...
# 通道注意力模块 (Channel Attention, CA)
class ChannelAttention(nn.Module):
    def __init__(self, inplanes, planes):
        super(ChannelAttention, self).__init__()
        self.att1 = nn.Sequential(
            nn.Conv2d(inplanes, int(planes / 8), 1, 1, bias=False),
            nn.BatchNorm2d(int(planes / 8), momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
        )
        self.att2 = nn.Conv2d(int(planes / 8), int(planes / 8), kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        att = torch.sigmoid(att)  # 生成通道权重
        return att * x  # 逐通道加权


# 空间注意力模块 (Spatial Attention, SA)
...
class BasicBlock_Attention(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.csa = CSAModule(planes, planes)  # 新的注意力模块
...
class StageModule(nn.Module):
    def __init__(self, input_branches, output_branches, c):
        ...
        for i in range(self.input_branches):  # 每个分支上都先通过4个BasicBlock_Attention
            w = c * (2 ** i)  # 对应第i个分支的通道数
            branch = nn.Sequential(
                BasicBlock_Attention(w, w),
                BasicBlock_Attention(w, w),
                BasicBlock_Attention(w, w),
                BasicBlock_Attention(w, w)
            )
...
class HighResolutionNet_CSA(nn.Module):
    # def __init__(self, base_channel: int = 32, num_joints: int = 17):
    def __init__(self, base_channel: int = 32, num_joints: int = 21):
        super().__init__()
        # Stem
