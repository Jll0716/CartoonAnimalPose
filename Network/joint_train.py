...
def calculate_lambda(epoch, mid_epoch, growth_rate):
    """
    计算动态调整的目标域权重 λ(t)
    :param epoch: 当前训练轮次
    :param mid_epoch: Sigmoid 中点（对应 T/2）
    :param growth_rate: Sigmoid 增长速率（对应 k）
    :return: 当前轮次的 λ(t)
    """
    return 0.4 + 0.6 / (1 + math.exp(-growth_rate * (epoch - mid_epoch)))
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mean_loss_source, lr_source = utils.train_one_epoch(model, optimizer, source_train_data_loader,
        ...
        target_loss_weight = calculate_lambda(epoch, mid_epoch, growth_rate)
        # 输出当前 λ 值，方便跟踪
        # print(f"Epoch {epoch}, Target Loss Weight (λ): {target_loss_weight:.4f}")

        mean_loss_target, lr_target = utils.train_one_epoch(model, optimizer, target_train_data_loader,
                                                            device=device, epoch=epoch,
                                                            print_freq=50, warmup=True,
                                                            scaler=scaler)
        target_loss.append(mean_loss_target.item())
        combined_loss = target_loss_weight * mean_loss_target.item() + mean_loss_source.item()
        ...
        with open(results_file, "a") as f:
            ...
            result_info = [f"{i:.4f}" for i in coco_info + [combined_loss]] + [f"{lr_target:.6f}"]
