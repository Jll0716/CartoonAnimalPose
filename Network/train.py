...
def create_model(num_joints, load_pretrain_weights=True):
    ...
    if load_pretrain_weights:
        ...
        # AnimalPose
        # weights_dict = torch.load("../Dataset/weight_dict/animalpose-apk10-mAP566-model299-20240911.pth", torch.device('cuda:0'))
        # Imagenet
        weights_dict = torch.load("../Dataset/weight_dict/cspnext-s_imagenet_600e-ea671761.pth", torch.device('cuda:0'))
        # HumanArt
        # weights_dict = torch.load("../Dataset/weight_dict/td-hm_hrnet-w32_8xb64-210e_humanart-256x192-0773ef0b_20230614.pth",torch.device('cuda:0'))  # noqa
        # APK-10
        # weights_dict = torch.load("./checkpoints/hrnet_w32_ap10k_256x256-18aac840_20211029.pth", torch.device('cuda:0'))
...
def main(args):
    ...
    # results_dir = f"./results/train_result/{args.model_type}-{dataset_name}-{args.epochs}-{current_time}/coco_info/"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-path', default='../Dataset/CAP4000', help='dataset')
    # COCO数据集人体关键点信息
