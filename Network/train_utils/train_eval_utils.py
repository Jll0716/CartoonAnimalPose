...
def train_one_epoch_joint_loss(model, optimizer, data_loader, device, epoch,
                    lamuda=1,print_freq=50, warmup=False, scaler=None):
    """
    Train the model for one epoch with combined source and target domain losses.
    """
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
...
def evaluate(model, data_loader, device, flip=False, flip_pairs=None):
    ...
    results_dir = "./results/train_result/" + current_time + "/"
    key_metric = EvalCOCOMetric(data_loader.dataset.coco, "keypoints", results_dir+"key_results.json")
