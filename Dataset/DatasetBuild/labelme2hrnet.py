...
# hrnet数据集格式：注释文件夹annotations，内含train_coco.json和val_coco.json。train文件夹，训练集图片。val文件夹，测试集图片。
# 在划分divide时，不仅将注释分开、同时将图片分开
# 创建annotations文件夹，并把生成的注释放于其中
...
def divide(dir, test_frac):
    ...
    os.chdir(os.path.join(dir, 'images'))
    for each in tqdm(train_files):
        each = each.replace('.json', '.png')
        if not os.path.exists(each):
            each = each.replace('.png', '.jpg')
        src_path = os.path.join(folder, each)
        dst_path = os.path.join(dir, train_images_folder, each)
        shutil.move(src_path, dst_path)
    ...
    for each in tqdm(val_files):
        ...
        if not os.path.exists(each):
            each = each.replace('.png', '.jpg')
        src_path = os.path.join(folder, each)
        dst_path = os.path.join(dir, val_images_folder, each)
        shutil.move(src_path, dst_path)
...
def to_coco(data_root):
    ...
    # 保存coco标注文件
    coco_path = 'annotations/train_coco.json'
    with open(coco_path, 'w') as f:
        json.dump(train_coco, f, indent=2)
    # 验证训练集
    my_coco = COCO('annotations/train_coco.json')
    print(my_coco)
    if my_coco:
        print("训练集转换完毕。")
    else:
    ...
    IMG_ID = 0
    ANN_ID = 0
    path = os.path.join(data_root, 'labelme_jsons', 'val_labelme_jsons')
    os.chdir(path)
    process_folder(val_coco, class_list)

    # 保存coco标注文件
    os.chdir('../../')
    coco_path = 'annotations/val_coco.json'
    with open(coco_path, 'w') as f:
        json.dump(val_coco, f, indent=2)
    #验证测试集
    my_coco = COCO('annotations/val_coco.json')
    print(my_coco)
    if my_coco:
        print("测试集转换完毕。")
    else:
...
if __name__ == '__main__':
    ...
    # 格式修改后的数据集目录
    data_trans_root = '../CartoonAnimal4000_HR'
